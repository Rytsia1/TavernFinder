...
### 2. Generate Seeded / Filtered NPC
`GET /api/npc/random?seed=<seed_value>&gender=<male|female>`

- **`seed`** (optional): Passing identical seed values (strings or numbers) **guarantees** the exact same NPC attributes every time.
- **`gender`** (optional): `'male'` or `'female'`. If omitted, gender is chosen deterministically by the PRNG.

**Example Queries:**
- `GET /api/npc/random?gender=female`
- `GET /api/npc/random?seed=elaris42&gender=male`
